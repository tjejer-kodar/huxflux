// Den här filen validerar huxflux-inputen. Du behöver
...
const isOnlyNumbers = (list) => {
  return list.some && !list.some((x) => typeof x !== "number");
};

const containsNumbersOutOfRange = (list) => {
  return (
...
const displaySuccess = () => {
  let failUl = document.getElementById("fail-ul");
  failUl.innerHTML = "";
...
export const validate = () => {
  ...
  if (!skip && !huxfluxes.every(isOnlyNumbers)) {
    errorMessages.push(
      '<span class="error-span">Listan får bara innehålla nummer</span>'
    );
    skip = true;
  }
  ...
  if (!skip && huxfluxes.some(hasDuplicates)) {
    errorMessages.push(
      '<span class="error-span">Det finns dubletter i listan</span>'
    );
  }

  if (!skip && huxfluxes.some(containsNumbersOutOfRange)) {
    errorMessages.push(
      '<span class="error-span">Det finns nummer i som inte är mellan 1-35</span>'
    );
  }

  if (!skip && huxfluxes.every((x) => x.join() === huxfluxes[0].join())) {
    errorMessages.push(
      '<span class="error-span">Funktionen ger samma värden varje gång</span>'
    );
  }
  ...
  if (!skip && lonelyNumbers.length) {
    errorMessages.push(
      '<span style="color: yellow">På hundra försök genererade funktionen aldrig talen ' +
        lonelyNumbers.join(", ") +
        ". Kontrollera din kod eller kör testet igen</span>"
    );
  }
